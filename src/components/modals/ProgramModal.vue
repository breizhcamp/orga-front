<template>
  ...
  >
    ...
    <div class="container-fluid minh">
      <div class="row d-flex align-items-center">
        ...
        >
          <div class="btn btn-sm btn-primary disabled">
            {{ hall.name }} <i>(Track {{ hall.trackId }})</i>
          </div>
          <button 
            type="button" 
        ...
        <div class="dropdown col-auto">
          <button 
            ...
            :disabled="halls.length === availableHalls.length"
          >Add Track</button>
      ...
      <div class="row mt-3" v-for="day in getEventDays(event)" :key="day.index">
        <div v-if="Array.from(slots.keys()).includes(day.index)" class="row text-center">
          ...
          <div class="row">
            <table style="width: 100%;" class="border">
              ...
              <tbody>
                <tr v-for="stamp in getTimestamps(slots.get(day.index)!, halls)" :key="stamp.time.unix()">
                  <td 
...
export default defineComponent({
  ...
  props: {
    ...
    availableHalls: { type: Array as PropType<Array<Hall>>, required: true },
    halls: { type: Array as PropType<Array<Hall>>, required: true },
  ...
  watch: {
    ...
    modalOpen() {
      if (!this.modalOpen) { this.$emit('close') }
    }
  },
  ...
  methods: {
    ...
    loadSlots() {
      try {
        axios.get('/konter/slots/event/' + this.event.id).then(
    ...
    addTrack(id: number) {
      axios.post(`/konter/halls/${id}/event/${this.event.id}`).then(
        () => this.$emit('reload')
      )
    },
    
    removeTrack(id: number) {
      axios.delete(`konter/halls/${id}/event/${this.event.id}`).then(
        () => this.$emit('reload')
      )
    },
